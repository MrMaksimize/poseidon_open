...
RABBITMQ_CREDS="${AIRFLOW_RMQ_USER}:${AIRFLOW_RMQ_PASS}"
BOOT_SCRIPT="/usr/local/airflow/poseidon/bootstrap.py"
...
if [ -e "/usr/local/airflow/poseidon/requirements.txt" ]; then
    $(which pip) install -r /usr/local/airflow/poseidon/requirements.txt
fi
